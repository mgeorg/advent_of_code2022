...
class Blueprint(object):
  def __init__(self, line):
    ...
    self.max_ore_robot = max(
        (self.clay_robot, self.obsidian_robot[0], self.geode_robot[0]))
    self.max_clay_robot = self.obsidian_robot[1]
  ...
  def evaluate(self, state, time_left):
    ...
    if state['ore_robot'] >= self.max_ore_robot:
      can_build_ore = False
    if state['clay_robot'] >= self.max_clay_robot:
...
for b in blueprints[:3]:
  memoized = dict()
  best = b.evaluate(initial_state, 32)
  print(f'{b.blueprint_id}: {best}  ({b.blueprint_id*best})')
  total *= best
